...
var HTMLCS_WCAG2AAA_Sniffs_Principle1_Guideline1_4_1_4_3_F24 = {
    ...
    testColourComboFail: function(element)
    {
        ...
        if (hasBg !== hasFg) {
            if (hasBg === true) {
                HTMLCS.addMessage(HTMLCS.WARNING, element, 'この要素は、インライン背景色か背景画像と補完関係のある継承された前景色を持っていることを確認してください。', 'F24.BGColour');
            } else {
                HTMLCS.addMessage(HTMLCS.WARNING, element, 'この要素は、インライン前景色か前景画像と補完関係のある継承された背景色または背景画像を持っていることを確認してください。', 'F24.FGColour');
            }
        }
    }
};
